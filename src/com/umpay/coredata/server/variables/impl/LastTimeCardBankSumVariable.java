 ...
 */
public class LastTimeCardBankSumVariable extends AbsVariable<DetailPo> {

	public LastTimeCardBankSumVariable(int index, String conditionStr, String mergeElems) {
		super(index, ConditionFactory.getCD(conditionStr), mergeElems);
	}
	public LastTimeCardBankSumVariable(int index, String conditionStr, String mergeElems,Pretreatmet pretreatmet) {
...
//			System.out.println(p.getDate() + ":" +new CardAmtInfo(p).getAmt());
	...
	private class CardAmtInfo {
		...
		public CardAmtInfo(DetailPo p) {
			if(p.getB()==null || "".equals(p.getB())){
				amt = null; 
			}else{
				amt = Math.abs(Double.valueOf(p.getB()).longValue());
			}
			date = p.getDate();
			time = p.getTime();
		}
		...
		public void compareDate2change(DetailPo p) {
			if(date.compareTo(p.getDate())<0){
				if(p.getB()==null || "".equals(p.getB())){
					...
					this.amt = Math.abs(Double.valueOf(p.getB()).longValue());
				}
				this.date=p.getDate();
				this.time=p.getTime();
